...
export async function getStaticPaths(context){
  ...
  const products = await db.collection("mouses").find({id:{$lt:6}},{projection:{_id:0}}).toArray()
...
export async function getStaticProps(context){
  ...
  const product = await db.collection("mouses").findOne({id: +context.params.productId},{projection:{_id:0}})

  const allProducts = await db.collection("mouses").find({},{projection:{_id:0}}).toArray()

  client.close();

  const analogs = []
  const analogMaxPrice = Math.round(product.price * ((100 - product.discount) / 100) * 1.25 ) + 1000
  ...
  allProducts.forEach((item)=>{
    ...
    if( itemPrice < analogMaxPrice && itemDpi > analogMinDpi && (itemWireless === product.wireless) && item.id !== product.id){
      analogs.push(item)
    }
  })
  return {
